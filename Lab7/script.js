      ...
      fetch('data/data.json')
        ...
        .then(categories => {
          categoriesContainer.innerHTML = categories.map(category => 
            `<a href="#" data-category="${category.shortname}" class="text-blue-600 hover:underline">${category.name}</a>`
          ).join(' ') + ' <a href="#" id="specials-link" class="text-blue-600 hover:underline">Specials</a>';
      ...
      fetch(`data/${category}.json`)
        ...
        .then(data => {
          contentContainer.innerHTML = `
            <h2 class="text-2xl font-bold mb-6">${data.category_name}</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              ${data.items.map(item => `
                <div class="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition">
                  ...
                  <p class="text-green-600 font-bold">Ціна: ${item.price} грн</p>
                </div>
              `).join('')}
            </div>
          `;
